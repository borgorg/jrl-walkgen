...
class TestInverseKinematics : public TestObject {
 ...
 protected:
  ...
  //    aof.setf(ios::scientific, ios::floatfield);
  //      aof << filterprecision( iteration * 0.005 ) << " "  ; // 1
  //      aof << filterprecision( m_OneStep.finalCOMPosition.roll[0] * M_PI
  //      /180) << " "  ; // 2 aof << filterprecision(
  //      m_OneStep.finalCOMPosition.pitch[0] * M_PI /180 ) << " "  ; // 3 aof
  //      << filterprecision( m_OneStep.finalCOMPosition.yaw[0] * M_PI /180 ) ;
  //      // 4 aof << endl ;
  //    aof.close();
  ...
  //    aof.setf(ios::scientific, ios::floatfield);
  //      aof << filterprecision( iteration * 0.005 ) << " "  ; // 1
  //      aof << filterprecision( m_OneStep.ZMPTarget(0) -
  //      m_CurrentConfiguration(0)) << " "  ; // 2 aof << filterprecision(
  //      m_OneStep.ZMPTarget(1) - m_CurrentConfiguration(1) ) << " "  ; // 3
  //      aof << filterprecision( aSupportState.z  - m_CurrentConfiguration(2))
  //      ; // 4 aof << endl ;
  //    aof.close();
