...
QPProblem::~QPProblem() { release_memory(); }

void QPProblem::release_memory() {}

void QPProblem::resize_all() {
  bool ok = false;
  ...
  if (ok) {
    ...
    war_.resize(2 * (3 * NbVariables_ * NbVariables_ / 2 + 10 * NbVariables_ +
                     2 * (NbConstraints_ + 1) + 20000),
                1, true);
  }
...
int QPProblem::reset_variant() {
  ...
  unsigned firstCol = 0;
  unsigned lastCol =
      (nbInvariantCols_ < Q_.NbCols_) ? nbInvariantCols_ : Q_.NbCols_;
  if ((firstRow >= Q_.NbRows_) || (firstCol >= Q_.NbCols_)) return 0;
...
void QPProblem::solve(solver_e Solver, solution_t &Result,
  ...
  iout_ = 0;
  iprint_ = 1;
  lwar_ = 2 * (3 * NbVariables_ * NbVariables_ / 2 + 10 * NbVariables_ +
               2 * (NbConstraints_ + 1) + 20000);
  ...
  switch (Solver) {
    case QLD:
      ...
      ql0001_(&m_, &me_, &mmax_, &n_, &nmax_, &mnn_, Q_dense_.Array_, D_.Array_,
              DU_dense_.Array_, DS_.Array_, XL_.Array_, XU_.Array_, X_.Array_,
              U_.Array_, &iout_, &ifail_, &iprint_, war_.Array_, &lwar_,
              iwar_.Array_, &liwar_, &eps_);
    ...
    case LSSOL:
      ...
      if (tests == CTR || tests == ALL) {
        ...
        for (unsigned i = 0; i < NbConstraints_; ++i) {
          if (tmp(i) + DS(i) < -1e-6) {
            std::cout << "Unrespected constraint " << i << " : " << tmp(i)
                      << " < " << -DS(i) << std::endl;
            ++nb_ctr;
          }
        }
        std::cout << std::endl
                  << "Nb unrespected constraints : " << nb_ctr << std::endl;
      }

      lssol_(&n_, &n_, &m_, &mmax_, &n_, DU_dense_.Array_, bl, bu, D_.Array_,
             istate_, kx_, X_.Array_, Q_dense_.Array_, b_, &inform_, &iter_,
             &obj_, clamda_, iwar_.Array_, &liwar_, war_.Array_, &lwar_);
...
void QPProblem::add_term_to(qp_element_e Type, const Eigen::MatrixXd &Mat,
  ...
  switch (Type) {
    case MATRIX_Q:
      Array_p = &Q_;
      NbVariables_ =
          (unsigned int)((col + Mat.cols() > NbVariables_) ? col + Mat.cols()
                                                           : NbVariables_);
      break;
    ...
    case MATRIX_DU:
      Array_p = &DU_;
      NbConstraints_ =
          (unsigned int)((row + Mat.rows() > NbConstraints_) ? row + Mat.rows()
                                                             : NbConstraints_);
      NbVariables_ =
          (unsigned int)((col + Mat.cols() > NbVariables_) ? col + Mat.cols()
                                                           : NbVariables_);
  ...
  if (NbVariables_ > Array_p->NbCols_) resize_all();
  ...
  unsigned warsize = 3 * NbVariables_ * NbVariables_ / 2 + 10 * NbVariables_ +
                     2 * (NbConstraints_ + 1) + 20000;
  if (warsize > war_.NbRows_) war_.resize(warsize, 1, true);
...
void QPProblem::add_term_to(qp_element_e Type, const Eigen::VectorXd &Vec,
  ...
  switch (Type) {
    case VECTOR_D:
      Array_p = &D_;
      NbVariables_ =
          (unsigned int)((row + Vec.size() > NbVariables_) ? row + Vec.size()
                                                           : NbVariables_);
      break;
    ...
    case VECTOR_DS:
      Array_p = &DS_;
      NbConstraints_ =
          (unsigned int)((row + Vec.size() > NbConstraints_) ? row + Vec.size()
                                                             : NbConstraints_);
      row++;  // The first rows of DU,DS are empty
      break;
  ...
  if ((NbVariables_ > D_.NbRows_) && (NbVariables_ > 0)) resize_all();
...
void QPProblem::dump_solver_parameters(std::ostream &aos) {
  aos << "m: " << m_ << std::endl
      << "me: " << me_ << std::endl
      << "mmax: " << mmax_ << std::endl
      << "n: " << n_ << std::endl
...
void QPProblem::dump(qp_element_e Type, const char *FileName) {
  std::ofstream aof;
...
void QPProblem::dump_problem(std::ostream &aos) {
  dump(MATRIX_Q, aos);
  dump(VECTOR_D, aos);
...
void QPProblem::dump(const char *FileName) {
  std::ofstream aof;
  aof.open(FileName, std::ofstream::out);
  dump_problem(aof);
  aof.close();
...
void QPProblem::dump(double Time) {
  char Buffer[1024];
