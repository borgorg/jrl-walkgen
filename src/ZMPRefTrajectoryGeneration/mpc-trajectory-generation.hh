...
class MPCTrajectoryGeneration : public SimplePlugin {
 ...
 public:
  ...
  void CurrentTime(double Time) { CurrentTime_ = Time; }

  /*! Get the current time. */
  double CurrentTime() { return CurrentTime_; }

  /// \brief Set the length of the preview window.
  inline void PreviewLength(double aPP) { PreviewControlTime_ = aPP; };

  /// \brief Get the preview control time window.
  inline const double &PreviewLength() const { return PreviewControlTime_; };

  /// \brief Return the Com Height.
  inline const double &ComHeight() const { return CoMHeight_; };

  /// \brief Return the Com Height.
  inline void ComHeight(double ComHeight) { CoMHeight_ = ComHeight; };
  ...
  inline void TDoubleSupport(double Tdble) { Tdble_ = Tdble; };
  ...
  /// \brief Set the sampling period for the preview.
  inline void NbPrwSamplings(int NbSamplings) { N_ = NbSamplings; };

  /// \brief Set the number of optimization parameters.
  inline void NbVariables(int NbVariables) { NbVariables_ = NbVariables; };

  /// \brief Set the number of optimization parameters.
  inline const int &NbVariables() const { return NbVariables_; };

  /// \brief Set the security margin for the zmp
  ///
  /// \param Margin
  inline void SecurityMargin(double Margin) { SecurityMargin_ = Margin; };

  /// \name Methods related to the generation of feet trajectories.
  /// @{
  /// \brief Returns the step height.
  inline const double &StepHeight() const { return StepHeight_; };
  ...
  inline const double &getOmega(void) const { return Omega_; };
  /// @}
