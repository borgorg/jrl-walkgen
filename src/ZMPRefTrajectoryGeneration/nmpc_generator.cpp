...
void NMPCgenerator::initNMPCgenerator(bool useLineSearch,
                                      support_state_t &currentSupport,
                                      COMState &lStartingCOMState,
                                      reference_t &local_vel_ref, unsigned N,
                                      unsigned nf, double T, double T_step) {
  N_ = N;
  nf_ = nf;
  ...
  qp_J_obs_i_.setZero();
  {
    for (unsigned int i = 0; i < F_kp1_x_.size();
         F_kp1_x_[i++] = currentSupport.X)
      ;
  };
  {
    for (unsigned int i = 0; i < F_kp1_y_.size();
         F_kp1_y_[i++] = currentSupport.Y)
      ;
  };
...
void NMPCgenerator::updateInitialCondition(
  ...
  updateCurrentSupport(time_, currentLeftFootAbsolutePosition,
...
#ifdef DEBUG_COUT
  cout << time_ << "  " << currentSupport_.StartTime << "  "
       << currentSupport_.TimeLimit << "  " << currentSupport_.NbStepsLeft
       << "  " << endl;
#endif
  ...
  updateSupportdeque(time_, currentLeftFootAbsolutePosition,
                     currentRightFootAbsolutePosition);
...
void NMPCgenerator::guessWarmStart() {
  if (Tfirst_ == T_ && currentSupport_.StateChanged) {
    ...
    if (nf_ == 1) {
      ...
      F_kp1_x_[nf_ - 1] = F_kp1_x_[nf_ - 2] + sign *
                                                  sin(F_kp1_theta_[nf_ - 2]) *
                                                  1.2 * FeetDistance_;
      F_kp1_y_[nf_ - 1] = F_kp1_y_[nf_ - 2] - sign *
                                                  cos(F_kp1_theta_[nf_ - 2]) *
                                                  1.2 * FeetDistance_;
    }
...
void NMPCgenerator::solve_qp() {
  // primal SQP solution
  QP_->solve(QuadProg_H_, QuadProg_g_, QuadProg_J_eq_, QuadProg_bJ_eq_,
...
void NMPCgenerator::getSolution(std::vector<double> &JerkX,
  ...
  if (currentSupport_.NbStepsLeft <= 0 &&
    ...
    FootStepX[0] =
        currentSupport_.X + sign * sin(currentSupport_.Yaw) * FeetDistance_;
    FootStepY[0] =
        currentSupport_.Y - sign * cos(currentSupport_.Yaw) * FeetDistance_;
    FootStepYaw[0] = currentSupport_.Yaw;
    for (unsigned i = 1; i < nf; ++i) {
      sign = -sign;
      FootStepX[i] =
          FootStepX[i - 1] + sign * sin(FootStepYaw[i - 1]) * FeetDistance_;
      FootStepY[i] =
          FootStepY[i - 1] - sign * cos(FootStepYaw[i - 1]) * FeetDistance_;
...
void NMPCgenerator::updateCurrentSupport(double time,
  ...
  cout << "previous support : \n"
       << currentSupport_.Phase << " " << currentSupport_.Foot << " "
       << currentSupport_.StepNumber << " " << currentSupport_.StateChanged
       << " " << currentSupport_.X << " " << currentSupport_.Y << " "
       << currentSupport_.Yaw << " " << currentSupport_.NbStepsLeft << " "
       << endl;
#endif
...
void NMPCgenerator::updateSupportdeque(double time,
  ...
  for (unsigned i = 0; i < SupportStates_deq_.size(); ++i) {
    cout << SupportStates_deq_[i].Phase << " " << SupportStates_deq_[i].Foot
         << " " << SupportStates_deq_[i].StepNumber << " "
         << SupportStates_deq_[i].StateChanged << " " << SupportStates_deq_[i].X
         << " " << SupportStates_deq_[i].Y << " " << SupportStates_deq_[i].Yaw
         << " " << SupportStates_deq_[i].NbStepsLeft << " " << endl;
  }
#endif
}
...
void NMPCgenerator::buildCoMCoPIntegrationMatrix() {
  ...
  for (unsigned i = 0; i < N_; ++i) {
    ...
    for (unsigned j = 0; j <= i; ++j) {
      i_j = (double)(i - j);
      if (j == 0) {
        Ppu_(i, j) = (T1 * T1 * T1 + 3 * i_j * T_ * T1 * T1 +
                      3 * i_j * i_j * T_ * T_ * T1) /
                     6.0;
        Pvu_(i, j) = T1 * T1 * 0.5 + i_j * T_ * T1;
        Pau_(i, j) = T1;
        Pzu_(i, j) = (T1 * T1 * T1 + 3 * i_j * T_ * T1 * T1 +
                      3 * i_j * i_j * T_ * T_ * T1) /
                         6.0 -
                     T1 * c_k_z_ / GRAVITY;
      } else {
...
void NMPCgenerator::updateCoMCoPIntegrationMatrix() {
  ...
  for (unsigned i = 0; i < N_; ++i) {
    ...
    Ppu_(i, 0) =
        (T1 * T1 * T1 + 3 * i * T_ * T1 * T1 + 3 * i * i * T_ * T_ * T1) / 6.0;
...
void NMPCgenerator::updateCoPconstraint(Eigen::VectorXd &U) {
  ...
  for (unsigned i = 0; i < N_; ++i) {
    ...
    rotMat_theta_(0, 0) = -sin(theta);
    rotMat_theta_(0, 1) = cos(theta);
    rotMat_theta_(1, 0) = -cos(theta);
    rotMat_theta_(1, 1) = -sin(theta);
    if (SupportStates_deq_[i + 1].Phase == DS) {
...
void NMPCgenerator::evalCoPconstraint(Eigen::VectorXd &U) {
  ...
  for (unsigned i = 0; i < N_; ++i) {
    double theta = theta_vec[SupportStates_deq_[i + 1].StepNumber];
    rotMat_xy_(0, 0) = cos(theta);
    rotMat_xy_(0, 1) = sin(theta);
    rotMat_xy_(1, 0) = -sin(theta);
    rotMat_xy_(1, 1) = cos(theta);
    if (SupportStates_deq_[i + 1].Phase == DS) {
...
void NMPCgenerator::updateStandingConstraint() {
  ...
  if (currentSupport_.Foot == LEFT) {
    evenPos_x =
        currentSupport_.X + sin(currentSupport_.Yaw) * RFI_->DSFeetDistance();
    evenPos_y =
        currentSupport_.Y - cos(currentSupport_.Yaw) * RFI_->DSFeetDistance();
...
void NMPCgenerator::initializeCostFunction() {
  // number of constraint
  nc_ = (unsigned int)(nc_cop_ + nc_foot_ + nc_vel_ + nc_rot_ + nc_obs_ +
                       nc_stan_);

  qp_H_.resize(nv_, nv_);
  qp_H_.setIdentity();
  qp_g_.resize(nv_);
  qp_g_.setZero();
  qp_g_x_.resize(N_ + nf_);
  qp_g_x_.setZero();
  qp_g_y_.resize(N_ + nf_);
  qp_g_y_.setZero();
  qp_g_theta_.resize(nf_);
  qp_g_theta_.setZero();
  Q_x_XX_.resize(N_, N_);
  Q_x_XX_.setZero();
  Q_x_XF_.resize(N_, nf_);
  Q_x_XF_.setZero();
  Q_x_FX_.resize(nf_, N_);
  Q_x_FX_.setZero();
  Q_x_FF_.resize(nf_, nf_);
  Q_x_FF_.setZero();
  Q_y_XX_.resize(N_, N_);
  Q_y_XX_.setZero();
  Q_theta_.resize(nf_, nf_);
  Q_theta_.setIdentity();
  p_xy_X_.resize(N_);
  p_xy_X_.setZero();
  p_xy_Fx_.resize(nf_);
  p_xy_Fx_.setZero();
...
void NMPCgenerator::updateCostFunction() {
  ...
  Q_x_XX_ = alpha_x_ * Pvu_.transpose() * Pvu_ +
            beta_ * Pzu_.transpose() * Pzu_ + minjerk_ * I_NN_;
  ...
  Q_x_FF_ = beta_ * V_kp1_.transpose() * V_kp1_ + delta_ * I_FF_ +
...
#ifdef DEBUG_COUT
  cout << vel_ref_.Global.X << " " << vel_ref_.Global.Y << endl;
#endif
  Pvsc_x_ = Pvs_ * c_k_x_;
  Pvsc_y_ = Pvs_ * c_k_y_;
...
#endif
  vel_ref_.Global.X_vec = Eigen::VectorXd::Constant(
      vel_ref_.Global.X_vec.size(), vel_ref_.Global.X);
  vel_ref_.Global.Y_vec = Eigen::VectorXd::Constant(
      vel_ref_.Global.Y_vec.size(), vel_ref_.Global.Y);
#ifdef DEBUG
  DumpVector("RefVectorX", vel_ref_.Global.X_vec);
  DumpVector("RefVectorY", vel_ref_.Global.Y_vec);
#endif
...
void NMPCgenerator::setGlobalVelocityReference(reference_t global_vel_ref) {
  ...
  vel_ref_.Local.Y = -vel_ref_.Global.X * sin(currentSupport_.Yaw) +
                     vel_ref_.Global.Y * cos(currentSupport_.Yaw);
  vel_ref_.Local.Yaw = vel_ref_.Global.Yaw;
  {
    for (unsigned int i = 0; i < vel_ref_.Global.X_vec.size();
         vel_ref_.Global.X_vec[i++] = vel_ref_.Global.X)
      ;
  };
  {
    for (unsigned int i = 0; i < vel_ref_.Global.Y_vec.size();
         vel_ref_.Global.Y_vec[i++] = vel_ref_.Global.Y)
      ;
  };
#ifdef DEBUG
  DumpVector("RefVectorX", vel_ref_.Global.X_vec);
