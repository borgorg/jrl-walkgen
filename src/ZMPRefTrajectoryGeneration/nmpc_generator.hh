...
class NMPCgenerator {
 ...
 public:
  ...
  inline void deleteAllObstacles() { obstacles_.clear(); }
  ...
  RelativeFeetInequalities *RFI() { return RFI_; }

  // Sampling period of the SQP preview
  inline double T() { return T_; }
  inline void T(double T) { T_ = T; }

  inline double Tfirst() { return Tfirst_; }

  // Number of sample in the SQP preview
  inline double N() { return N_; }
  inline void N(unsigned N) { N_ = N; }

  // Number of planned foot step in the SQP preview
  inline double nf() { return nf_; }
  inline void nf(unsigned nf) { nf_ = nf; }
