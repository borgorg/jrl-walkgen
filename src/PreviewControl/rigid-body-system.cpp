...
RigidBodySystem::RigidBodySystem(SimplePluginManager *SPM, PinocchioRobot *aPR,
                                 SupportFSM *FSM)
    : mass_(0),
      CoMHeight_(0),
      T_(0),
...
int RigidBodySystem::initialize_trajectories() {
  ...
  OFTG_->SetParameters(FootTrajectoryGenerationStandard::Z_AXIS, SSPeriod / 2.0,
                       OFTG_->GetStepHeight());
  ...
  for (unsigned int i = 0; i < NbInstantsSS; i++) {
    if (i * T_ > SSPeriod / 2.0) {
      ...
      OFTG_->SetParameters(FootTrajectoryGenerationStandard::Z_AXIS,
                           SSPeriod / 2.0, 0.0, OFTG_->GetStepHeight(), 0.0,
                           0.0);
    }

    FTIt->Z(0) = OFTG_->Compute(FootTrajectoryGenerationStandard::Z_AXIS,
                                i * T_ - LocalStartTime);
    FTIt->Z(2) = OFTG_->ComputeSecDerivative(
  ...
  for (TrajIt = CoM_.Trajectory().begin(); TrajIt < CoM_.Trajectory().end();
       ++TrajIt) {
  ...
  std::deque<support_state_t>::iterator ST_it = SupportTrajectory_deq_.begin();
  ST_it->X = 0.257792;
  ST_it->Y = -0.105;
  ST_it++;
  ST_it->X = 0.257792;
...
int RigidBodySystem::precompute_trajectories(
  ...
  deque<rigid_body_state_t>::iterator LFTraj_it =
      LeftFoot_.Trajectory().begin();
  deque<rigid_body_state_t>::iterator RFTraj_it =
      RightFoot_.Trajectory().begin();
  deque<rigid_body_state_t>::iterator FFTraj_it =
      FlyingFootTrajectory_deq_.begin();
  for (unsigned int i = 0; i < N_; i++) {
    ...
    unsigned int endLoop = std::min<unsigned>(
        SS_it->NbInstants, (unsigned int)FlyingFootTrajectory_deq_.size());
    for (unsigned int j = 0; j < endLoop; j++) FFTraj_it++;
...
int RigidBodySystem::update(
  ...
  if (multiBody_) {
    ...
    compute_foot_pol_dynamics(SupportStates_deq,
                              LeftFoot_.Dynamics(ACCELERATION),
...
int RigidBodySystem::compute_dyn_cop(unsigned nbSteps) {
  ...
  for (unsigned int i = 0; i < N_; i++) {
    ...
    CoPDynamicsJerk_.S.row(i) += CoM_.Dynamics(POSITION).S.row(i) *
                                 CoM_.Mass() * (CoMTraj_it->Z[2] + GRAVITY) /
                                 GRF;
    CoPDynamicsJerk_.U.row(i) += CoM_.Dynamics(POSITION).U.row(i) *
                                 CoM_.Mass() * (CoMTraj_it->Z[2] + GRAVITY) /
                                 GRF;
    CoPDynamicsJerk_.UT.col(i) += CoM_.Dynamics(POSITION).U.row(i) *
                                  CoM_.Mass() * (CoMTraj_it->Z[2] + GRAVITY) /
                                  GRF;
    ...
    if (multiBody_) {
      LeftFoot_.Dynamics(COP_POSITION).S.row(i) +=
          LeftFoot_.Dynamics(POSITION).S.row(i) * LeftFoot_.Mass() *
          (LFTraj_it->Z[2] + GRAVITY) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).U.row(i) +=
          LeftFoot_.Dynamics(POSITION).U.row(i) * LeftFoot_.Mass() *
          (LFTraj_it->Z[2] + GRAVITY) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).UT.col(i) +=
          LeftFoot_.Dynamics(POSITION).U.row(i) * LeftFoot_.Mass() *
          (LFTraj_it->Z[2] + GRAVITY) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).S.row(i) -=
          LeftFoot_.Dynamics(ACCELERATION).S.row(i) * LeftFoot_.Mass() *
          (LFTraj_it->Z[0]) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).U.row(i) -=
          LeftFoot_.Dynamics(ACCELERATION).U.row(i) * LeftFoot_.Mass() *
          (LFTraj_it->Z[0]) / GRF;
      LeftFoot_.Dynamics(COP_POSITION).UT.col(i) -=
          LeftFoot_.Dynamics(ACCELERATION).U.row(i) * LeftFoot_.Mass() *
          (LFTraj_it->Z[0]) / GRF;
      ...
      RightFoot_.Dynamics(COP_POSITION).S.row(i) +=
          RightFoot_.Dynamics(POSITION).S.row(i) * RightFoot_.Mass() *
          (RFTraj_it->Z[2] + GRAVITY) / GRF;
      RightFoot_.Dynamics(COP_POSITION).U.row(i) +=
          RightFoot_.Dynamics(POSITION).U.row(i) * RightFoot_.Mass() *
          (RFTraj_it->Z[2] + GRAVITY) / GRF;
      RightFoot_.Dynamics(COP_POSITION).UT.col(i) +=
          RightFoot_.Dynamics(POSITION).U.row(i) * RightFoot_.Mass() *
          (RFTraj_it->Z[2] + GRAVITY) / GRF;
      RightFoot_.Dynamics(COP_POSITION).S.row(i) -=
          RightFoot_.Dynamics(ACCELERATION).S.row(i) * RightFoot_.Mass() *
          (RFTraj_it->Z[0]) / GRF;
      RightFoot_.Dynamics(COP_POSITION).U.row(i) -=
          RightFoot_.Dynamics(ACCELERATION).U.row(i) * RightFoot_.Mass() *
          (RFTraj_it->Z[0]) / GRF;
      RightFoot_.Dynamics(COP_POSITION).UT.col(i) -=
          RightFoot_.Dynamics(ACCELERATION).U.row(i) * RightFoot_.Mass() *
          (RFTraj_it->Z[0]) / GRF;
...
int RigidBodySystem::compute_dyn_cjerk(linear_dynamics_t &Dynamics) {
  ...
  switch (Dynamics.Type) {
    ...
    case COP_POSITION:
      for (unsigned int i = 0; i < N_; i++) {
        ...
        Dynamics.S(i, 1) = (i + 1) * T_;
        Dynamics.S(i, 2) =
            (i + 1) * (i + 1) * T_ * T_ * 0.5 - CoMHeight_ / 9.81;
        for (unsigned int j = 0; j < N_; j++)
          if (j <= i)
            Dynamics.U(i, j) = Dynamics.UT(j, i) =
    ...
    if (SS_it->Phase == DS) {
      ...
      if (SS_it->StepNumber == 0 && SS_it->StepNumber < nbSteps) {
        if (FFDynamics->Type == POSITION) {
          ...
          FFDynamics->U(i, SS_it->StepNumber) =
              FFDynamics->UT(SS_it->StepNumber, i) = Uabar[0];
          SFDynamics->U(i, SS_it->StepNumber) =
              SFDynamics->UT(SS_it->StepNumber, i) = 0.0;
        }
        if (((SS_it->NbInstants) * T_ > FSM_->StepPeriod() - T_) &&
          ...
          FFDynamics->S(i, 2) = FFDynamics->S(i - 1, 2);
          SFDynamics->S(i, 2) = SFDynamics->S(i - 1, 2);
          FFDynamics->U(i, SS_it->StepNumber) = FFDynamics->UT(
              SS_it->StepNumber, i) = FFDynamics->U(i - 1, SS_it->StepNumber);
          SFDynamics->U(i, SS_it->StepNumber) = SFDynamics->UT(
              SS_it->StepNumber, i) = SFDynamics->U(i - 1, SS_it->StepNumber);
        }
      } else if (SS_it->StepNumber == 1 && SS_it->StepNumber < nbSteps) {
        if (FFDynamics->Type == POSITION) {
          FFDynamics->S(i, 0) = Spbar[0];
        ...
        if ((SS_it->NbInstants) * T_ > FSM_->StepPeriod() - T_) {
          ...
          FFDynamics->U(i, SS_it->StepNumber) = FFDynamics->UT(
              SS_it->StepNumber, i) = FFDynamics->U(i - 1, SS_it->StepNumber);
          SFDynamics->U(i, SS_it->StepNumber - 1) =
              SFDynamics->UT(SS_it->StepNumber - 1, i) =
                  SFDynamics->U(i - 1, SS_it->StepNumber - 1);
        }
      } else if (SS_it->StepNumber == 2) {
        FFDynamics->S(i, 0) = FFDynamics->S(i - 1, 0);
        SFDynamics->S(i, 0) = SFDynamics->S(i - 1, 0);
...
int RigidBodySystem::generate_trajectories(
    double Time, const solution_t &Solution,
