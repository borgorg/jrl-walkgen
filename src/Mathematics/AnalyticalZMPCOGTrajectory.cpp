...
AnalyticalZMPCOGTrajectory::~AnalyticalZMPCOGTrajectory() { FreePolynomes(); }
...
void AnalyticalZMPCOGTrajectory::SetNumberOfIntervals(
...
bool AnalyticalZMPCOGTrajectory::ComputeCOM(double t, double &r) {
  ...
  for (unsigned int j = 0; j < m_DeltaTj.size(); j++) {
    ...
    if (((t + m_Sensitivity) >= reftime) &&
      ...
      ODEBUG(" coefficients hyperbolique : "
             << r << " m_omegaj[" << j << "]=" << m_omegaj[j] << " deltaj "
             << deltaj << " m_V[" << j << "]:" << m_V[j] << " m_W[" << j
             << "]:" << m_W[j] << " m_T[" << j << "]:" << m_DeltaTj[j]);
      if (m_ListOfCOGPolynomials[j] != 0) {
        double add2r;
        add2r = m_ListOfCOGPolynomials[j]->Compute(deltaj);
        ODEBUG(" add2r: " << add2r << " "
                          << "Polynomial (" << j << ") with degree (supposed:"
...
bool AnalyticalZMPCOGTrajectory::ComputeCOMSpeed(double t, double &r) {
  ...
  for (unsigned int j = 0; j < m_DeltaTj.size(); j++) {
    ...
    if (((t + m_Sensitivity) >= reftime) &&
      ...
      ODEBUG(" coefficients hyperbolique : "
             << r << " m_omegaj[j]=" << m_omegaj[j] << " deltaj " << deltaj
             << " m_V[j]:" << m_V[j] << " m_W[j]:" << m_W[j]);
      if (m_ListOfCOGPolynomials[j] != 0) {
        double add2r;
...
bool AnalyticalZMPCOGTrajectory::ComputeCOM(double t, double &r, int j) {
  ...
  r = cosh(m_omegaj[j] * deltaj) * m_V[j] + sinh(m_omegaj[j] * deltaj) * m_W[j];
  ...
  for (unsigned int j = 0; j < m_DeltaTj.size(); j++) {
    if (((t + m_Sensitivity) >= reftime) &&
      ...
      if (m_ListOfZMPPolynomials[j] != 0) {
        ...
        ODEBUG(" add2r: " << add2r << " ");
        ODEBUG("Polynomial (" << j << ") with degree (supposed:"
                              << m_PolynomialDegree[j] << ")");
        ODEBUG("Coefficients:");
...
void AnalyticalZMPCOGTrajectory::SetStartingTimeIntervalsAndHeightVariation(
    vector<double> &lTj, vector<double> &lomegaj) {
  if ((int)lTj.size() == m_NbOfIntervals) {
    ...
    cerr << "Pb while initializing the time intervals. " << lTj.size() << " "
...
void AnalyticalZMPCOGTrajectory::SetPolynomialDegrees(
    vector<unsigned int> &lPolynomialDegree) {
...
void AnalyticalZMPCOGTrajectory::GetPolynomialDegrees(
    vector<unsigned int> &lPolynomialDegree) const {
  lPolynomialDegree = m_PolynomialDegree;
}

void AnalyticalZMPCOGTrajectory::GetNumberOfIntervals(
    unsigned int &lNbOfIntervals) const {
  lNbOfIntervals = m_NbOfIntervals;
}

void AnalyticalZMPCOGTrajectory::GetHyperbolicCoefficients(
    vector<double> &lV, vector<double> &lW) const {
  lV = m_V;
  lW = m_W;
}

void AnalyticalZMPCOGTrajectory::GetStartingPointAndHeightVariation(
    vector<double> &lTj, vector<double> &lomegaj) {
  lTj = m_DeltaTj;
  lomegaj = m_omegaj;
}

bool AnalyticalZMPCOGTrajectory::GetFromListOfCOGPolynomials(
    unsigned int j, Polynome *&aPoly) const {
  aPoly = 0;
...
bool AnalyticalZMPCOGTrajectory::GetFromListOfZMPPolynomials(
    unsigned int j, Polynome *&aPoly) const {
  aPoly = 0;
...
void AnalyticalZMPCOGTrajectory::
  ...
  if ((m_ListOfCOGPolynomials[intervalindex] != 0) &&
    ...
    for (unsigned int i = 0; i < m_PolynomialDegree[intervalindex] - 1; ++i)
      CoefsForZMP[i] =
          CoefsFromCOG[i] -
          ((double)(i + 1.0) * (double)(i + 2.0)) * (CoefsFromCOG[i + 2] / sTc);
    m_ListOfZMPPolynomials[intervalindex]->SetCoefficients(CoefsForZMP);
  }
}

void AnalyticalZMPCOGTrajectory::TransfertCoefficientsFromCOGTrajectoryToZMPOne(
    vector<double> &lCoMZ, vector<double> &lZMPZ) {
  for (int j = 0; j < m_NbOfIntervals; j++) {
    double lCoMZ2 = lCoMZ[j];
    double lZMPZ2 = lZMPZ[j];
    TransfertOneIntervalCoefficientsFromCOGTrajectoryToZMPOne(j, lCoMZ2,
                                                              lZMPZ2);
  }
}

void AnalyticalZMPCOGTrajectory::Building3rdOrderPolynomial(
    unsigned int anIntervalj, double pjTjm1, double pjTj) {
...
double AnalyticalZMPCOGTrajectory::FluctuationMaximal() {
  ...
  Tmax = m_AbsoluteTimeReference -
         2 * CoefsForZMP[2] * m_DeltaTj[0] /
...
bool AnalyticalZMPCOGTrajectory::GetIntervalIndexFromTime(
    double t, unsigned int &j, unsigned int &prev_j) {
