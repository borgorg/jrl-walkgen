...
PatternGeneratorInterfacePrivate::PatternGeneratorInterfacePrivate(
    PinocchioRobot *aPinocchioRobotRobot)
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++)
      if (i == j)
        m_MotionAbsPos(i, j) = m_MotionAbsOrientation(i, j) =
            m_WaistAbsPos(i, j) = m_WaistRelativePos(i, j) = 1.0;
      else
        m_MotionAbsPos(i, j) = m_MotionAbsOrientation(i, j) =
            m_WaistAbsPos(i, j) = m_WaistRelativePos(i, j) = 0.0;
  }
...
#define number_of_method 18
  std::string aMethodName[number_of_method] = {":LimitsFeasibility",
                                               ":ZMPShiftParameters",
...
void PatternGeneratorInterfacePrivate::ObjectsInstanciation() {
  ...
  m_FeetTrajectoryGenerator = new LeftAndRightFootTrajectoryGenerationMultiple(
  ...
  m_StOvPl = new StepOverPlanner(m_ObstaclePars, m_PinocchioRobot);
  ...
  m_DoubleStagePCStrategy->SetBufferPositions(&m_ZMPPositions, &m_COMBuffer,
                                              &m_LeftFootPositions,
                                              &m_RightFootPositions);

  m_CoMAndFootOnlyStrategy = new CoMAndFootOnlyStrategy(this);
  m_CoMAndFootOnlyStrategy->SetBufferPositions(&m_ZMPPositions, &m_COMBuffer,
                                               &m_LeftFootPositions,
                                               &m_RightFootPositions);
...
void PatternGeneratorInterfacePrivate::InterObjectRelationInitialization() {
  ...
  m_DoubleStagePCStrategy->InitInterObjects(
      m_PinocchioRobot, m_ComAndFootRealization[0], m_StepStackHandler);

  m_CoMAndFootOnlyStrategy->InitInterObjects(
      m_PinocchioRobot, m_ComAndFootRealization, m_StepStackHandler);
  ...
  for (vector<ComAndFootRealization *>::iterator CFR_it =
...
void PatternGeneratorInterfacePrivate::m_SetLimitsFeasibility(
    istringstream &strm) {
  while (!strm.eof()) {
    if (!strm.eof()) {
...
void PatternGeneratorInterfacePrivate::ReadSequenceOfSteps(
    istringstream &strm) {
...
void PatternGeneratorInterfacePrivate::setCoMPerturbationForce(
    istringstream &strm) {
  // Read the data inside strm.
...
void PatternGeneratorInterfacePrivate::initOnlineHerdt() {
  ...
  EvaluateStartingState(lStartingCOMState, lStartingZMPPosition,
  ...
  m_ZMPVRQP->InitOnLine(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions,
                        m_RightFootPositions, InitLeftFootAbsPos,
...
void PatternGeneratorInterfacePrivate::initOnlineNaveau() {
  ...
  m_ZMPVRSQP->InitOnLine(m_ZMPPositions, m_COMBuffer, m_LeftFootPositions,
                         m_RightFootPositions, InitLeftFootAbsPos,
                         InitRightFootAbsPos, RelativeFootPositions,
                         lStartingCOMState, lStartingZMPPosition);
#endif
...
void PatternGeneratorInterfacePrivate::EvaluateStartingCOM(
    Eigen::VectorXd &Configuration, Eigen::Vector3d &lStartingCOMState) {
...
void PatternGeneratorInterfacePrivate::EvaluateStartingState(
  ...
  m_GlobalStrategyManager->EvaluateStartingState(
      lBodyInit, lStartingCOMState, lStartingZMPPosition, lStartingWaistPose,
      InitLeftFootAbsPos, InitRightFootAbsPos);

  ostringstream osscomheightcmd;
  osscomheightcmd << ":comheight " << lStartingCOMState.z[0];
...
void PatternGeneratorInterfacePrivate::AutomaticallyAddFirstStep(
  ...
  ODEBUG("InitLeftFoot:" << InitLeftFootAbsPos.x << " " << InitLeftFootAbsPos.y
  ...
  ODEBUG("AutomaticallyAddFirstStep: " << aRFP.sx << " " << aRFP.sy << " "
                                       << aRFP.theta);

  lRelativeFootPositions.push_front(aRFP);
}

void PatternGeneratorInterfacePrivate::CommonInitializationOfWalking(
    COMState &lStartingCOMState, Eigen::Vector3d &lStartingZMPPosition,
    Eigen::VectorXd &BodyAnglesIni, FootAbsolutePosition &InitLeftFootAbsPos,
    FootAbsolutePosition &InitRightFootAbsPos,
    deque<RelativeFootPosition> &lRelativeFootPositions,
    vector<double> &lCurrentJointValues, bool ClearStepStackHandler) {
  ...
  m_StepStackHandler->CopyRelativeFootPosition(lRelativeFootPositions,
                                               ClearStepStackHandler);

  for (unsigned int i = 0; i < lRelativeFootPositions.size(); i++) {
    ODEBUG(lRelativeFootPositions[i].sx << " " << lRelativeFootPositions[i].sy
  ...
  Eigen::Matrix<double, 6, 1> lStartingWaistPose;
  m_GlobalStrategyManager->EvaluateStartingState(
      BodyAnglesIni, lStartingCOMState, lStartingZMPPosition,
      lStartingWaistPose, InitLeftFootAbsPos, InitRightFootAbsPos);

  // Add the first step automatically when the corresponding
  // option is set on.
  if (m_AutoFirstStep) {
    AutomaticallyAddFirstStep(lRelativeFootPositions, InitLeftFootAbsPos,
                              InitRightFootAbsPos, lStartingCOMState);
    ...
    if (aof.is_open()) {
      for (unsigned int i = 0; i < lRelativeFootPositions.size(); i++) {
        aof << lRelativeFootPositions[i].sx << " "
            ...
            << lRelativeFootPositions[i].theta << endl;
      }
    }
  }
}
...
void PatternGeneratorInterfacePrivate::StartOnLineStepSequencing() {
  ...
  CommonInitializationOfWalking(lStartingCOMState, lStartingZMPPosition,
                                BodyAnglesIni, InitLeftFootAbsPos,
                                InitRightFootAbsPos, lRelativeFootPositions,
                                lCurrentJointValues, false);
  ...
  ODEBUG("StartOnLineStepSequencing - 3 "
         << lStartingCOMState.x[0] << " " << lStartingCOMState.y[0] << " "
         << lStartingCOMState.z[0] << " " << lRelativeFootPositions.size());
  ...
  if (m_AlgorithmforZMPCOM == ZMPCOM_KAJITA_2003) {
    ...
    NbOfStepsToRemoveFromTheStack = m_ZMPD->InitOnLine(
        m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions,
        InitLeftFootAbsPos, InitRightFootAbsPos, lRelativeFootPositions,
        lStartingCOMState, lStartingZMPPosition);
    ...
    NbOfStepsToRemoveFromTheStack = m_ZMPM->InitOnLine(
        m_ZMPPositions, m_COMBuffer, m_LeftFootPositions, m_RightFootPositions,
        InitLeftFootAbsPos, InitRightFootAbsPos, lRelativeFootPositions,
        lStartingCOMState, lStartingZMPPosition);
  ...
  ODEBUG("StartOnLineStepSequencing - 4 "
         << m_ZMPPositions.size() << " " << m_LeftFootPositions.size() << " "
...
void PatternGeneratorInterfacePrivate::FinishAndRealizeStepSequence() {
  ...
  CreateZMPReferences(lRelativeFootPositions, lStartingCOMState,
                      lStartingZMPPosition, InitLeftFootAbsPos,
                      InitRightFootAbsPos);

  ODEBUG("First m_ZMPPositions" << m_ZMPPositions[0].px << " "
                                << m_ZMPPositions[0].py);
  ...
  if (0) {
    m_ZMPD->DumpDataFiles("/tmp/ZMPSetup.dat",
                          "/tmp/LeftFootAbsolutePosSetup.dat", m_ZMPPositions,
                          m_LeftFootPositions);

    m_ZMPD->DumpDataFiles("/tmp/ZMPSetup.dat",
                          "/tmp/RightFootAbsolutePosSetup.dat", m_ZMPPositions,
                          m_RightFootPositions);
  }
  ...
  ODEBUG4("m_ZMPPositions : "
              << m_ZMPPositions.size() << endl
              << " m_LeftFootPositions: " << m_LeftFootPositions.size() << endl
              << " m_RightFootPositions: " << m_RightFootPositions.size()
              << endl
  ...
  if (m_StepStackHandler->GetWalkMode() == 2) {
    ...
    m_StOvPl->PolyPlanner(m_COMBuffer, m_LeftFootPositions,
  ...
  m_count = 0;
  ODEBUG(
      "FinishAndRealizeStepSequence() - 8 - COMBuffer: " << m_COMBuffer.size());
...
void PatternGeneratorInterfacePrivate::ChangeOnLineStep(istringstream &strm,
                                                        double &newtime) {
...
void PatternGeneratorInterfacePrivate::m_SetAlgoForZMPTraj(
    istringstream &strm) {
  string ZMPTrajAlgo;
  strm >> ZMPTrajAlgo;
...
void PatternGeneratorInterfacePrivate::m_SetUpperBodyMotionParameters(
    istringstream &strm) {
...
bool PatternGeneratorInterfacePrivate::RunOneStepOfTheControlLoop(
  ...
  m_Running = RunOneStepOfTheControlLoop(
      CurrentConfiguration, CurrentVelocity, CurrentAcceleration, ZMPTarget,
      finalCOMState, LeftFootPosition, RightFootPosition);
  return m_Running;
}
...
bool PatternGeneratorInterfacePrivate::RunOneStepOfTheControlLoop(
  ...
  m_Running = RunOneStepOfTheControlLoop(
      CurrentConfiguration, CurrentVelocity, CurrentAcceleration, ZMPTarget,
      aCOMRefState, LeftFootPosition, RightFootPosition);
...
bool PatternGeneratorInterfacePrivate::RunOneStepOfTheControlLoop(
    Eigen::VectorXd &CurrentConfiguration, Eigen::VectorXd &CurrentVelocity,
    Eigen::VectorXd &CurrentAcceleration, Eigen::VectorXd &ZMPTarget,
    COMPosition &finalCOMPosition, FootAbsolutePosition &LeftFootPosition,
    FootAbsolutePosition &RightFootPosition) {
  COMState aCOMState;
  m_Running = RunOneStepOfTheControlLoop(
      CurrentConfiguration, CurrentVelocity, CurrentAcceleration, ZMPTarget,
      aCOMState, LeftFootPosition, RightFootPosition);
...
bool PatternGeneratorInterfacePrivate::RunOneStepOfTheControlLoop(
  ...
  if (m_StepStackHandler->IsOnLineSteppingOn()) {
    ...
    if (m_AlgorithmforZMPCOM == ZMPCOM_WIEBER_2006) {
      ...
      m_ZMPD->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer,
                     m_LeftFootPositions, m_RightFootPositions);
    } else if (m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) {
      ODEBUG("InternalClock:" << m_InternalClock
                              << " SamplingPeriod: " << m_SamplingPeriod);

      m_ZMPM->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer,
                     m_LeftFootPositions, m_RightFootPositions);
    }
  } else
  ...
  if (m_AlgorithmforZMPCOM == ZMPCOM_HERDT_2010) {
    m_ZMPVRQP->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer,
                      m_LeftFootPositions, m_RightFootPositions);
  ...
  if (m_AlgorithmforZMPCOM == ZMPCOM_NAVEAU_2015) {
    if (m_feedBackControl) {
      m_ZMPVRSQP->UpdateCoM(ZMPTarget, finalCOMState);
    ...
    m_ZMPVRSQP->OnLine(m_InternalClock, m_ZMPPositions, m_COMBuffer,
                       m_LeftFootPositions, m_RightFootPositions);
    m_Running = m_ZMPVRSQP->Running();

    m_ZMPVRSQP->UpdateCurrentPos(m_ZMPPositions[0], m_COMBuffer[0],
                                 m_LeftFootPositions[0],
                                 m_RightFootPositions[0]);
  }
#endif

  m_GlobalStrategyManager->OneGlobalStepOfControl(
      LeftFootPosition, RightFootPosition, ZMPTarget, finalCOMState,
      CurrentConfiguration, CurrentVelocity, CurrentAcceleration);

  ODEBUG("finalCOMState: "
         << finalCOMState.x[0] << " " << finalCOMState.x[1] << " "
         << finalCOMState.x[2] << " " << finalCOMState.y[0] << " "
         << finalCOMState.y[1] << " " << finalCOMState.y[2] << " "
         << finalCOMState.z[0] << " " << finalCOMState.z[1] << " "
         << finalCOMState.z[2] << " " << finalCOMState.yaw[0] << " "
         << finalCOMState.yaw[1] << " " << finalCOMState.yaw[2] << " "
         << finalCOMState.pitch[0] << " " << finalCOMState.pitch[1] << " "
         << finalCOMState.pitch[2] << " " << finalCOMState.roll[0] << " "
         << finalCOMState.roll[1] << " " << finalCOMState.roll[2] << " ");
  ...
  ODEBUG4("CurrentWaistState: " << m_CurrentWaistState.x[0] << " "
  ...
  if (m_GlobalStrategyManager->EndOfMotion() ==
    ...
    if (m_StepStackHandler->IsOnLineSteppingOn()) {
      ...
      if (m_StepStackHandler->ReturnStackSize() <= 1) {
        m_StepStackHandler->AddStandardOnLineStep(m_NewStep, m_NewStepX,
                                                  m_NewStepY, m_NewTheta);
        m_NewStep = false;
      }
      ...
      bool EnoughSteps =
          m_StepStackHandler->ReturnFrontFootPosition(lRelativeFootPositions);
      if ((!EnoughSteps) && (!EndSequence)) {
        std::cerr << "You don't have enough steps in the"
                  << " step stack handler." << std::endl;
        std::cerr << "And this is not an end sequence." << std::endl;
      }
      ...
      if (!EndSequence) {
        ...
        if (m_AlgorithmforZMPCOM == ZMPCOM_WIEBER_2006) {
          ...
          ODEBUG("Putting a new step SX: " << lRelativeFootPositions.sx
                                           << " SY: "
                                           << lRelativeFootPositions.sy);
          m_ZMPM->SetCurrentTime(m_InternalClock);
          m_ZMPM->OnLineAddFoot(lRelativeFootPositions, m_ZMPPositions,
                                m_COMBuffer, m_LeftFootPositions,
                                m_RightFootPositions, EndSequence);
        ...
        if (m_AlgorithmforZMPCOM == ZMPCOM_WIEBER_2006) {
          ...
          m_ZMPD->EndPhaseOfTheWalking(m_ZMPPositions, m_COMBuffer,
                                       m_LeftFootPositions,
                                       m_RightFootPositions);
        } else if (m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) {
          ODEBUG("Putting a new step SX: " << lRelativeFootPositions.sx
                                           ...
                                           << lRelativeFootPositions.sy);
          m_ZMPM->SetCurrentTime(m_InternalClock);
          m_ZMPM->EndPhaseOfTheWalking(m_ZMPPositions, m_COMBuffer,
                                       m_LeftFootPositions,
                                       m_RightFootPositions);
...
void PatternGeneratorInterfacePrivate::m_SetTimeDistrParameters(
    istringstream &strm) {
...
void PatternGeneratorInterfacePrivate::SetCurrentJointValues(
    Eigen::VectorXd &lCurrentJointValues) {
...
void PatternGeneratorInterfacePrivate::m_FinishAndRealizeStepSequence(
    istringstream &strm) {
  int Synchronize = 1;
...
void PatternGeneratorInterfacePrivate::AddOnLineStep(double X, double Y,
                                                     double Theta) {
  m_NewStep = true;
  m_NewStepX = X;
  m_NewStepY = Y;
...
void PatternGeneratorInterfacePrivate::UpdateAbsolutePosition(
...
void PatternGeneratorInterfacePrivate::getWaistPositionMatrix(
    Eigen::Matrix4d &lWaistAbsPos) const {
  lWaistAbsPos = m_WaistAbsPos;
}

// TODO test me
void PatternGeneratorInterfacePrivate::getWaistPositionAndOrientation(
...
void PatternGeneratorInterfacePrivate::setWaistPositionAndOrientation(
    double aTQ[7]) {
  // Position
...
void PatternGeneratorInterfacePrivate::getWaistVelocity(double &dx, double &dy,
                                                        double &omega) const {
...
void PatternGeneratorInterfacePrivate::setVelocityReference(double x, double y,
                                                            double yaw) {
...
int PatternGeneratorInterfacePrivate::ChangeOnLineStep(
    double time, FootAbsolutePosition &aFootAbsolutePosition, double &newtime) {
  /* Compute the index of the interval which will be modified. */
  if (m_AlgorithmforZMPCOM == ZMPCOM_MORISAWA_2007) {
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPM->OnLineFootChange(m_InternalClock + time, aFootAbsolutePosition,
                             m_ZMPPositions, m_COMBuffer, m_LeftFootPositions,
                             m_RightFootPositions, m_StepStackHandler);
    vector<double> lDj;
...
int PatternGeneratorInterfacePrivate::CreateZMPReferences(
    deque<RelativeFootPosition> &lRelativeFootPositions,
    COMState &lStartingCOMState, Eigen::Vector3d &lStartingZMPPosition,
    FootAbsolutePosition &InitLeftFootAbsPos,
    FootAbsolutePosition &InitRightFootAbsPos) {
  ...
  //   }
    ...
    m_COMBuffer.clear();
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPVRQP->GetZMPDiscretization(
        m_ZMPPositions, m_COMBuffer, lRelativeFootPositions,
        m_LeftFootPositions, m_RightFootPositions, m_Xmax, lStartingCOMState,
        lStartingZMPPosition, InitLeftFootAbsPos, InitRightFootAbsPos);
  }
    ...
    m_COMBuffer.clear();
    m_ZMPM->SetCurrentTime(m_InternalClock);
    m_ZMPVRSQP->GetZMPDiscretization(
        m_ZMPPositions, m_COMBuffer, lRelativeFootPositions,
        m_LeftFootPositions, m_RightFootPositions, m_Xmax, lStartingCOMState,
        lStartingZMPPosition, InitLeftFootAbsPos, InitRightFootAbsPos);
  }
...
void PatternGeneratorInterfacePrivate::AddStepInStack(double dx, double dy,
                                                      double theta) {
  if (m_StepStackHandler != 0) {
    m_StepStackHandler->AddStepInTheStack(dx, dy, theta, m_TSsupport,
                                          m_TDsupport);
  }
}

void PatternGeneratorInterfacePrivate::setZMPInitialPoint(
    Eigen::Vector3d &lZMPInitialPoint) {
  m_ZMPInitialPoint = lZMPInitialPoint;
  m_ZMPInitialPointSet = true;
}

void PatternGeneratorInterfacePrivate::getZMPInitialPoint(
    Eigen::Vector3d &lZMPInitialPoint) const {
